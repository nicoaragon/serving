...
func InitializeLogger() {
	...
	loggerInitializeOnce.Do(func() {
		...
		// Set Proxy Verbosities
		k := flag.NewFlagSet("klogDummy", flag.ExitOnError)
		klog.InitFlags(k)
		k.Lookup("v").Value.Set(fmt.Sprintf("%d", Verbosity))

		// TODO(coryrc): must hold off until all repos are using istio/glog proxy
